...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.BLOCKAGE_TIME_BEFORE_YESTERDAY, {
      ...getContext(request)
    })
  },
  post: async (request, h) => {
    ...
    const { dateString } = request.yar.get(constants.redisKeys.BLOCKAGE_DATE_BEFORE_YESTERDAY)

    // validate payload for errors
    const errorSummary = validatePayload(dateString, time)
    if (errorSummary.errorList.length > 0) {
      const { dateWordString } = request.yar.get(constants.redisKeys.BLOCKAGE_DATE_BEFORE_YESTERDAY)
      return h.view(constants.views.BLOCKAGE_TIME_BEFORE_YESTERDAY, {
        errorSummary,
        dateWordString,
        ...request.payload
    ...
    request.yar.set(constants.redisKeys.BLOCKAGE_TIME_BEFORE_YESTERDAY, formattedTime)
...
const validatePayload = (dateString, time) => {
  ...
  } else if (formattedTime !== 'INVALID_TIME_FORMAT' && !getDateTime(dateString, formattedTime).isBefore(moment())) {
    errorSummary.errorList.push({
...
const getContext = request => {
  const time = request.yar.get(constants.redisKeys.BLOCKAGE_TIME_BEFORE_YESTERDAY)
  const { dateWordString } = request.yar.get(constants.redisKeys.BLOCKAGE_DATE_BEFORE_YESTERDAY)
  return {
    time,
    dateWordString
...
export default [
  {
    method: 'GET',
    path: constants.routes.BLOCKAGE_TIME_BEFORE_YESTERDAY,
