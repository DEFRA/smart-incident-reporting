...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.BLOCKAGE_DATE_BEFORE_YESTERDAY, {
      fieldErrorClasses,
      getDateErrors,
  ...
  post: async (request, h) => {
    ...
    request.yar.set(constants.redisKeys.BLOCKAGE_DATE_BEFORE_YESTERDAY, { dateString, dateWordString, payload })
    return h.redirect(constants.routes.BLOCKAGE_TIME_BEFORE_YESTERDAY)
  }
}

const getContext = request => {
  const answers = request.yar.get(constants.redisKeys.BLOCKAGE_DATE_BEFORE_YESTERDAY)
...
export default [
  {
    method: 'GET',
    path: constants.routes.BLOCKAGE_DATE_BEFORE_YESTERDAY,
