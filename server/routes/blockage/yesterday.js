...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.BLOCKAGE_YESTERDAY, {
      ...getContext(request)
    })
  },
  post: async (request, h) => {
    ...
    request.yar.set(constants.redisKeys.BLOCKAGE_YESTERDAY, formattedTime)
...
const getContext = request => {
  const time = request.yar.get(constants.redisKeys.BLOCKAGE_YESTERDAY)
  return {
    time
  }
}
...
export default [
  {
    method: 'GET',
    path: constants.routes.BLOCKAGE_YESTERDAY,
