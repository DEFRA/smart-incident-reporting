...
const question = questionSets.BLOCKAGE.questions.BLOCKAGE_LOCATION_MAP
const baseAnswer = {
...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.BLOCKAGE_LOCATION_MAP, {
      ...getContext(request)
    })
  },
  post: async (request, h) => {
    ...
    // handle redirects
    return h.redirect(constants.routes.BLOCKAGE_LOCATION_DESCRIPTION_OPTIONAL)
  }
}

const getContext = request => {
  const locationOption = request.yar.get(constants.redisKeys.BLOCKAGE_LOCATION_OPTION)
  const showCurrentLocation = locationOption?.[0]?.answerId === questionSets.BLOCKAGE.questions.BLOCKAGE_LOCATION_OPTION.answers.gps.answerId
  const location = request.yar.get(constants.redisKeys.BLOCKAGE_LOCATION_MAP)
...
export default [
  {
    method: 'GET',
    path: constants.routes.BLOCKAGE_LOCATION_MAP,
