...
const createYesterdayRoutes = ({ _problem, route, redirect }) => {
  const handlers = {
    get: async (request, h) => {
      return h.view(constants.views.RARS_YESTERDAY, {
        ...getContext(request)
      })
    },
    post: async (request, h) => {
      ...
      request.yar.set(constants.redisKeys.RARS_YESTERDAY, formattedTime)
  ...
  const getContext = request => {
    const time = request.yar.get(constants.redisKeys.RARS_YESTERDAY)
    return {
      time
    }
  }
