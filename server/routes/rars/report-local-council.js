...
const pestsQuestion = 'Report the {pests} to your local council'
...
const createReportLocalCouncilRoutes = ({ problem, route }) => {
  ...
  const handlers = {
    get: async (request, h) => {
      const { title, pageTitle } = titleHelper(request, question, pestsQuestion, problem)
      const issue = problem === 'pests'
