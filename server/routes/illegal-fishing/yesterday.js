...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.ILLEGAL_FISHING_YESTERDAY, {
      ...getContext(request)
    })
  },
  post: async (request, h) => {
    ...
    request.yar.set(constants.redisKeys.ILLEGAL_FISHING_YESTERDAY, formattedTime)
...
const getContext = request => {
  const time = request.yar.get(constants.redisKeys.ILLEGAL_FISHING_YESTERDAY)
  return {
    time
  }
}
...
export default [
  {
    method: 'GET',
    path: constants.routes.ILLEGAL_FISHING_YESTERDAY,
