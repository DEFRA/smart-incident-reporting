...
const imageQuestion = questionSets.ILLEGAL_FISHING.questions.ILLEGAL_FISHING_IMAGES_OR_VIDEO
...
const isEmailRequired = (request) => {
  const imagesOrVideoAnswer = request.yar.get(constants.redisKeys.ILLEGAL_FISHING_IMAGES_OR_VIDEO)
...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.ILLEGAL_FISHING_CONTACT_DETAILS, {
      ...getContext(request),
...
const getContext = request => {
  const contactDetails = request.yar.get(constants.redisKeys.ILLEGAL_FISHING_CONTACT_DETAILS)
...
export default [
  {
    method: 'GET',
    path: constants.routes.ILLEGAL_FISHING_CONTACT_DETAILS,
