...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.ILLEGAL_FISHING_DATE_BEFORE_YESTERDAY, {
      fieldErrorClasses,
      getDateErrors,
  ...
  post: async (request, h) => {
    ...
    request.yar.set(constants.redisKeys.ILLEGAL_FISHING_DATE_BEFORE_YESTERDAY, { dateString, dateWordString, payload })
    return h.redirect(constants.routes.ILLEGAL_FISHING_TIME_BEFORE_YESTERDAY)
  }
}

const getContext = request => {
  const answers = request.yar.get(constants.redisKeys.ILLEGAL_FISHING_DATE_BEFORE_YESTERDAY)
...
export default [
  {
    method: 'GET',
    path: constants.routes.ILLEGAL_FISHING_DATE_BEFORE_YESTERDAY,
