...
const header = 'What date did you see this?'
...
describe(url, () => {
  ...
  describe('POST', () => {
    it('Happy: accepts valid date and redirects to blockage/time-before-yesterday', async () => {
      const options = {
        url,
        payload: {
          'date-day': '20',
      ...
      expect(response.request.yar.get(constants.redisKeys.BLOCKAGE_DATE_BEFORE_YESTERDAY)).toEqual({ dateString, dateWordString, payload })
    })
