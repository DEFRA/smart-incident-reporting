...
const url = constants.routes.BLOCKAGE_TIME_BEFORE_YESTERDAY
...
describe(url, () => {
  describe('GET', () => {
    ...
    it(`Should return success response and correct view with prefilled data for ${url}`, async () => {
      const sessionData = {
        'blockage/date-before-yesterday': { dateString, dateWordString },
        'blockage/time-before-yesterday': '9:30am'
      }
  ...
  describe('POST', () => {
    ...
    it('Sad: datetime must be in the past', async () => {
      ...
      const sessionData = {
        'blockage/date-before-yesterday': { dateString: todayDateString, dateWordString: todayDateWordString }
      }
      const options = {
        url,
        payload: {
