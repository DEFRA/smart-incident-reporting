...
describe(url, () => {
  ...
  describe('POST', () => {
    it('Happy: accept and store a point as a national grid reference', async () => {
      ...
      expect(response.headers.location).toEqual(constants.routes.BLOCKAGE_LOCATION_DESCRIPTION_OPTIONAL)
      expect(response.request.yar.get(constants.redisKeys.BLOCKAGE_LOCATION_MAP)).toEqual([{
        ...baseAnswer,
