...
const url = constants.routes.PESTS_TYPE
...
describe(url, () => {
  describe('GET', () => {
    ...
    it('Should show the correct service name and link for a pests service page', async () => {
      ...
      expect(html.querySelector('.govuk-service-navigation__service-name').textContent).toContain(constants.serviceNames.PESTS)
      expect(serviceNameLink.getAttribute('href')).toBe(constants.urls.GOV_UK_PESTS)
  ...
  describe('POST', () => {
    ...
    it('Should redirect to pests source and only set selected type when flies selected', async () => {
      const options = { url, payload: { answerId: question.answers.flies.answerId.toString() } }
      const response = await submitPostRequest(options)
      expect(response.headers.location).toEqual(constants.routes.PESTS_SOURCE)
      expect(response.request.yar.get(question.key)).toBeNull()
      expect(response.request.yar.get(constants.redisKeys.PESTS_TYPE_SELECTED)).toEqual(question.answers.flies.text.toLowerCase())
    })
    ...
    it.each([
      ...
      expect(response.request.yar.get(constants.redisKeys.PESTS_TYPE_SELECTED)).toEqual(expectedSelectedType)
    })
  })
})
