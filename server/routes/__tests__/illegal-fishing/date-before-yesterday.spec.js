...
const url = constants.routes.ILLEGAL_FISHING_DATE_BEFORE_YESTERDAY
...
describe(url, () => {
  describe('GET', () => {
    ...
    it(`Should return success response and correct view with prefilled data for ${url}`, async () => {
      ...
      const sessionData = {
        'illegal-fishing/date-before-yesterday': { payload }
      }
  ...
  describe('POST', () => {
    it('Happy: accepts valid date and redirects to illegal-fishing/time-before-yesterday', async () => {
      const options = {
        url,
        payload: {
          'date-day': '20',
      ...
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_TIME_BEFORE_YESTERDAY)
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_DATE_BEFORE_YESTERDAY)).toEqual({ dateString, dateWordString, payload })
    })
