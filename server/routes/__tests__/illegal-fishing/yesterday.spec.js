...
describe(url, () => {
  describe('GET', () => {
    ...
    it(`Should return success response and correct view with prefilled data for ${url}`, async () => {
      const sessionData = {
        'illegal-fishing/yesterday': '9:30am'
      }
  ...
  describe('POST', () => {
    it('Happy: accepts valid answers and redirects to illegal-fishing/people-description', async () => {
      ...
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_YESTERDAY)).toEqual('9:30am')
    ...
    it('Happy: accepts valid answers with single digit time and redirects to illegal-fishing/people-description', async () => {
      ...
      const response = await submitPostRequest(sessionData)
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_PEOPLE_DESCRIPTION)
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_YESTERDAY)).toEqual('1:05am')
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_WHEN)).toEqual(dateTime.toISOString())
    })
