...
const url = constants.routes.ILLEGAL_FISHING_YESTERDAY
...
describe(url, () => {
  describe('GET', () => {
    ...
    it(`Should return success response and correct view with prefilled data for ${url}`, async () => {
      const sessionData = {
        'illegal-fishing/yesterday': '09:30am'
      }
  ...
  describe('POST', () => {
    it('Happy: accepts valid answers and redirects to illegal-fishing/people-description', async () => {
      const time = '09:30am'
      ...
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_YESTERDAY)).toEqual('09:30am')
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_WHEN)).toEqual(dateTime.toISOString())
    })
    it('Happy: accepts valid answers with single digit time and redirects to illegal-fishing/people-description', async () => {
      const time = '1:5am'
      const dateTime = getDateTime(time)
      const sessionData = {
        url,
      ...
      const response = await submitPostRequest(sessionData)
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_PEOPLE_DESCRIPTION)
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_YESTERDAY)).toEqual('1:5am')
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_WHEN)).toEqual(dateTime.toISOString())
    })
    it('Happy: accepts valid answers with all caps period and redirects to smell/pollution-substance', async () => {
      ...
      const response = await submitPostRequest(sessionData)
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_PEOPLE_DESCRIPTION)
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_YESTERDAY)).toEqual('12:30AM')
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_WHEN)).toEqual(dateTime.toISOString())
    })
