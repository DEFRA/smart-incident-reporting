...
const url = constants.routes.ILLEGAL_FISHING_LOCATION_DESCRIPTION
...
describe(url, () => {
  ...
  describe('POST', () => {
    it('Happy: accept and store a location description', async () => {
      const locationDescription = 'This is a description of the location of the ILLEGAL_FISHING'
      const options = {
        url,
        payload: {
          locationDescription
      ...
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_WHEN)
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_LOCATION_DESCRIPTION)).toEqual([{
        ...baseAnswer,
    ...
    it('Sad: errors when locationDescription exceeds the character limit', async () => {
      ...
      expect(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_LOCATION_DESCRIPTION)).toBeFalsy()
    })
