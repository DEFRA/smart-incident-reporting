...
const url = constants.routes.ILLEGAL_FISHING_WHEN
...
describe(url, () => {
  ...
  describe('POST', () => {
    it('Happy: accepts valid answerId now and redirects to illegal-fishing/people-fishing', async () => {
      const answerId = 1
      const options = {
        url,
        payload: {
      ...
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_PEOPLE_FISHING)
      expect(new Date(response.request.yar.get(constants.redisKeys.ILLEGAL_FISHING_WHEN))).toBeInstanceOf(Date)
    })
    it('Happy: accepts valid answerId of earlier today and redirects to illegal-fishing/earlier-today', async () => {
      const answerId = 2
      const options = {
        url,
        payload: {
      ...
      const response = await submitPostRequest(options)
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_EARLIER_TODAY)
    })
    it('Happy: accepts valid answerId of yesterday and redirects to illegal-fishing/yesterday', async () => {
      const answerId = 3
      const options = {
        url,
        payload: {
      ...
      const response = await submitPostRequest(options)
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_YESTERDAY)
    })
    it('Happy: accepts valid answerId of before yesterday and redirects to illegal-fishing/date-before-yesterday', async () => {
      const answerId = 4
      const options = {
        url,
        payload: {
      ...
      const response = await submitPostRequest(options)
      expect(response.headers.location).toEqual(constants.routes.ILLEGAL_FISHING_DATE_BEFORE_YESTERDAY)
    })
    it('Sad: errors on no answerId', async () => {
      ...
      expect(response.payload).toContain('Select when you noticed this')
    })
  })
})
