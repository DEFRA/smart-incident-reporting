...
const url = constants.routes.VERMIN_SOURCE
const question = questionSets.REPORT_REGULATED_SITE.questions.RARS_SOURCE
const header = 'Where is the vermin coming from?'
const baseAnswer = {
...
describe(url, () => {
  ...
  describe('POST', () => {
    ...
    it('Sad: no radio selected, returns error state', async () => {
      ...
      expect(response.payload).toContain('Select a type of place or activity where the vermin is coming from')
    })
  })
})
