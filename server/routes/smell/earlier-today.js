...
const handlers = {
  get: async (request, h) => {
    return h.view(constants.views.SMELL_EARLIER_TODAY, {
      ...getContext(request)
    })
  },
  post: async (request, h) => {
    ...
    request.yar.set(constants.redisKeys.SMELL_EARLIER_TODAY, formattedTime)
...
const getContext = request => {
  const time = request.yar.get(constants.redisKeys.SMELL_EARLIER_TODAY)
  return {
    time
  }
}
...
export default [
  {
    method: 'GET',
    path: constants.routes.SMELL_EARLIER_TODAY,
